...
order by TITAN_VI_FORMULARIOS.ESTADO asc ";
...
while ($row = mysqli_fetch_array($result,MYSQLI_NUM)){
    ...
    if(empty($row[9])){
        ...
        $obstec = utf8_encode("$row[13]");
        $obstec = preg_replace('/[ ]+/', ' ', $obstec);
    ...
    <td><button class="btn btn-warning 	glyphicon glyphicon-pencil" onclick="abrirModalajax(\''.$row[4].'\',\''.$row[0].'\' ,\''.$obstec.'\',\''.$preguntaFinal.'\'
    ,\''.$areaf.'\',\''.utf8_decode($nombrefinal).'\',\''.utf8_decode($row[15]).'\',\''.utf8_decode($row[16]).'\')">  
    Editar</button></td>
